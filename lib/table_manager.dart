...
class _TableManagerState extends State<TableManager> {
  ...
  void checkPersonalization() async {
    ...
    _personalizationEnabled = response.body == "True" ? true : false;
    setState(() {});
    return;
  }
  ...
  void updateData([Timer? state]) async {
    ...
    late Response tableJson;
    late Response userPreferencesJson;
    try {
      tableJson = await httpClient.get(apiURI([
        "table",
        widget.selectedTableID,
        widget.selectedTableDailyAccessCode,
        "get_data",
      ]));
    ...
    if (tableJson.statusCode != 200) {
      widget.deselectTable("Table credentials have expired");
      return;
    }
  ...
  Widget build(BuildContext context) {
    double px = MediaQuery.of(context).size.shortestSide / 1080;
          ...
          children: [
            Expanded(
              ...
              children: [
                ...
                IconButton(
                  onPressed: () =>
                      setTableHeight(_selectedTableData!.currentHeight + 0.1),
  ...
  void savePresets(List<HeightValue> presets) async {
    ...
    try {
      response = await httpClient.post(
        ...
        body: (jsonEncode(preferences)).toString(),
      );
    } catch (e) {
      widget.deselectTable();
      return;

...
class _TableManagerState extends State<TableManager> {
  ...
  Timer? _updateData;
  TableData? _selectedTableData;
  UserPreferences? _userPreferences;
  double _retryCounter = 0;

  double percentHeight() =>
      (_selectedTableData!.currentHeight - _selectedTableData!.minHeight) /
      (_selectedTableData!.maxHeight - _selectedTableData!.minHeight);

  void checkPersonalization() async {
    MasasasResponse response = await MasasasApi.getUserPersonalizationState(
        widget.userID, widget.userDailyAccessCode);

    switch (response.result) {
      case MasasasResult.ok:
        _personalizationEnabled =
            bool.parse(response.body, caseSensitive: false);
        setState(() {});
        return;
      case MasasasResult.badRequest:
        widget.invalidateUserCredentials(response.body);
        return;
      case MasasasResult.connectionError:
        widget.deselectTable(response.body);
        return;
    }
  }
  ...
  void updateData([Timer? state]) async {
    ...
    switch ((tableJson.result, userPreferencesJson.result)) {
      ...
      case (MasasasResult.ok, MasasasResult.ok):
        _retryCounter = 0;
        _selectedTableData = TableData.fromJson(jsonDecode(tableJson.body));
        _userPreferences =
              ...
              children: [
                Center(
                  child: Text.rich(
                    ...
                    TextSpan(
                      text: "${_selectedTableData!.location}\n",
                  ...
                  children: [
                    IconButton(
                      onPressed: () => setTableHeight(
                          _selectedTableData!.currentHeight + 0.1),
                      ...
                      child: Stack(
                        children: [
                          ...
                          AnimatedPositioned(
                            ...
                            child: Center(
                              child: Text(
                                "${_selectedTableData!.currentHeight.toStringAsFixed(2)} m",
                      ...
                      child: ReorderableListView.builder(
                        ...
                        itemBuilder: (context, index) => ListTile(
                          ...
                          leading: _personalizationEnabled
                              ...
                              : null,
                          title: Text(_userPreferences!.heightPresets[index]
                              .toString()),
                          trailing: _personalizationEnabled
                              ...
                              : null,
                          onTap: () {
                            setTableHeight(_userPreferences!
                                .heightPresets[index]
                                .toAbsoluteHeight(_selectedTableData!.minHeight,
                                    _selectedTableData!.maxHeight));
                          },
                        ),
                      ),
                    ),
  ...
  void setTableHeight(double val) async {
    MasasasResponse response = await MasasasApi.setTableHeight(
        widget.selectedTableID, widget.selectedTableDailyAccessCode, val);
    switch (response.result) {
      case MasasasResult.ok:
        _selectedTableData!.currentHeight = double.parse(response.body);
        return setState(() {});
  ...
  void addPreset(HeightValue val) {
    savePresets(_userPreferences!.heightPresets.toList()..add(val));
  }

  void savePresets(List<HeightValue> presets) async {
    UserPreferences preferences =
        UserPreferences(_userPreferences!.name, presets);
    MasasasResponse response = await MasasasApi.setPreferences(widget.userID,
        widget.userDailyAccessCode, jsonEncode(preferences.toJson()));

    switch (response.result) {
      case MasasasResult.ok:
        _userPreferences = UserPreferences.fromJson(jsonDecode(response.body));
        return setState(() {});

      case MasasasResult.badRequest:
        if (response.body == "User personalization is disabled") {
